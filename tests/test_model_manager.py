...
def test_train_model(train_config, train_data):
    ...
    saved_config_path = Path(trained_model.train_config.save_dir) / "config.yaml"
    ...
    saved_lgb_path = Path(trained_model.train_config.save_dir) / "lgb_model.txt"
...
def test_predict_model(predict_data, predict_config):
    ...
    saved_preds = pd.read_csv(predict_config.preds_path)
