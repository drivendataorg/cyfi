...
def test_generate_features(train_data, features_config, satellite_meta):
    ...
    assert features[["TMP_min", "SPFH_mean"]].notna().all().all()
    ...
    assert np.isclose(features.loc["9c601f226c2af07d570134127a7fda27", "TMP_min"], 292.83050)
...
def test_download_climate_data(tmp_path, train_data, features_config):
